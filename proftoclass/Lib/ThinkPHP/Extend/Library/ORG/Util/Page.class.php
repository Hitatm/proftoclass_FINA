...
class Page {
    ...
    protected $coolPages   ;
    // 分页显示定制
    protected $config  =    array('prev'=>'«','next'=>'»','header'=>'条</span>','theme'=>'%upPage%  %linkPage% %downPage%  <span> <label class="total">%nowPage%/%totalPage% 页</label> <label class="total">共 %totalRow% %header% </label>');
    // 默认分页变量名
    protected $varPage;
        ...
        for($i=1;$i<=$this->rollPage;$i++){
            ...
            if($page!=$this->nowPage){
                ...
                if($this->totalPages != 1){
                    $linkPage .= "<a title='' class='c'>".$page."</a>";
                }
            }
        }
        $pageStr     =   str_replace(
