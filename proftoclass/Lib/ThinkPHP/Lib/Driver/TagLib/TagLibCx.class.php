...
class TagLibCx extends TagLib {
    ...
    protected $tags   =  array(
		...
		'plugin'    =>  array('attr'=>'name','close'=>0,'level'=>'1'),
        );
    ...
    public function _plugin($attr){
		...
		//获取插件路径
        $path='./Public/Admin/Plugin/'.$name.'/default.php';
        if(!file_exists($path)){
            return '<b style=color:red >插件'.$name.'不存在，请检查插件列表！</b>';;
        }
		...
		//如果未开启则提示错误
        if($Plugin->param['state']==1){
			return '<font color=red >插件'.$name.'未开启，请在插件列表中开启！</font>';
		}elseif($Plugin->param['state']==null){
            return '<font color=red >插件'.$name.'未注册，请在插件列表中进行注册！</font>';
        }else{
			return $Plugin->run();
		}
    }
